class AddActiveSavedAndInReminderProjectsInProjectsView < ActiveRecord::Migration[6.1]
  def change
    execute <<-SQL
      ...
      AS $function$
        SELECT
          COALESCE($1.state_acronym, (SELECT a.address_state FROM users u JOIN addresses a ON u.address_id = a.id WHERE u.id = $1.project_user_id)) = (SELECT a.address_state FROM users u JOIN addresses a ON u.address_id = a.id WHERE u.id = current_user_id());
      $function$;
